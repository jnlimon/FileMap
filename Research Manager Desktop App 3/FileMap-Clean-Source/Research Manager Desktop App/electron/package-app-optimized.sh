...
# FileMap Optimized App Packager
# This script creates a minimal .app bundle with only essential files

set -e

echo "🚀 Creating optimized FileMap.app..."
...
# Copy only the essential Electron binary (not the entire Electron.app)
...
# Copy only essential frameworks (much smaller than full Electron.app)
...
cp -r "../node_modules/.electron-fmH6LRjN/dist/Electron.app/Contents/Frameworks/Squirrel.framework" "$FRAMEWORKS_DIR/" 2>/dev/null || true
